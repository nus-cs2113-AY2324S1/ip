...
public class Lemon {
        ...
        try {
            ...
            while (scanner.hasNextLine()) {
                ...
                case "D":
                    ...
                    break;
                case "E":
                    addNewDeadline(String.format("event %s /from %s /to %s", taskStr[2], taskStr[3], taskStr[4]),
                            isDone, filePath);
                    break;
                default:
    ...
    public static void deleteFromFile(String filePath, int deleteLineSerialNo) {
        try {
            ...
            if (file.delete()) {
                if (!tempFile.renameTo(file)) {
                    throw new IOException("Uh-oh! An error has occurred while deleting from file.\n");
                }
            } else {
                throw new IOException("Uh-oh! An error has occurred while deleting from file.\n");
            }
        } catch (IOException e) {
            System.out.println("Uh-oh! An error has occurred while deleting from file.\n");
        }
    }
